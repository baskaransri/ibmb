...
def maybe_to_numpy(v: Union[np.ndarray, torch.Tensor]):
...
def topk_ppr_matrix(
    ...
    _, indptr, out_degree = np.unique(
        edge_index_np[0], return_index=True, return_counts=True
    )
    indptr = np.append(indptr, len(edge_index_np[0]))

    neighbors, weights = calc_ppr_topk_parallel(
        indptr, edge_index_np[1], out_degree, alpha, eps, output_node_indices
    )

    # neighbors, weights = get_calc_ppr()(indptr, edge_index_np[1], out_degree, alpha, eps)

    ppr_matrix = construct_sparse(
        neighbors, weights, (len(output_node_indices), num_nodes)
    )
    ...
    if normalization == "sym":
        ...
        ppr_matrix.data = (
            deg_sqrt[output_node_indices[row]] * ppr_matrix.data * deg_inv_sqrt[col]
        )
    elif normalization == "col":
        # Assume undirected (symmetric) adjacency matrix
        deg_inv = 1.0 / np.maximum(out_degree, 1e-12)

        row, col = ppr_matrix.nonzero()
        ppr_matrix.data = (
            out_degree[output_node_indices[row]] * ppr_matrix.data * deg_inv[col]
        )
...
def construct_sparse(neighbors, weights, shape):
    i = np.repeat(
        np.arange(len(neighbors)), np.fromiter(map(len, neighbors), dtype=int)
    )
...
def calc_ppr_topk_parallel(
    indptr, indices, deg, alpha, epsilon, nodes
...
def get_partitions(
    edge_index: Union[torch.LongTensor, SparseTensor],
    num_partitions: int,
    ...
    assert isinstance(
        edge_index, (torch.LongTensor, SparseTensor)
    ), f"Unsupported edge_index type {type(edge_index)}"
    ...
    _, partptr, perm = edge_index.partition(
...
# BASKARAN: cleanup for my own understanding
# perhaps look at clusterloader?
def get_partitions2(
    edge_index: Union[torch.LongTensor, SparseTensor],
    num_partitions: int,
    indices: torch.LongTensor,
    ...
    partitions = [perm[i:j] for i, j in pairwise(partptr)]
    # partitions = []
    # for i in range(len(partptr) - 1):
    #     partitions.append(perm[partptr[i] : partptr[i + 1]])
